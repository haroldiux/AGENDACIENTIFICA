...
def get_sedes(db: Session = Depends(deps.get_db), skip: int = 0, limit: int = 100):
...
def create_sede(sede: SedeCreate, db: Session = Depends(deps.get_db)):
...
def get_sede(sede_id: int, db: Session = Depends(deps.get_db)):
...
def delete_sede(sede_id: int, db: Session = Depends(deps.get_db)):
    sede = db.query(Sede).filter(Sede.id == sede_id).first()
    if not sede:
        raise HTTPException(status_code=404, detail="Sede not found")
    db.delete(sede)
